# Finalization handover: two completed seeds and pending seed 2026
...
| Recovery workflow | Finalized-only discovery, interruption accounting, and null-selection handling were decided and implemented | `docs/issue-16-` through `issue-22-` decision records |
...
## Results from the two completed seeds
...
The two completed seeds support a concise negative finding: under this exact frozen QLoRA intervention and protocol, both seeds showed visible gains but produced no capability-eligible checkpoint. The visible composite is strongly driven by Open Prompt Injection; it is not evidence of uniform safety improvement.

The evidence does **not** support a capability-preserving mitigation claim, a held-out InjecAgent generalization claim, a causal explanation for capability loss, or a population-level inference from two seeds. Fixed-example bootstrap intervals are conditional on the evaluated examples; cross-seed mean/range/standard deviation are descriptive only.

## Current outcome of Issue #23 / seed 2026

GitHub issue #23 ("Conditionally execute and verify seed 2026") remains open. Its real evidence run has not finalized the required training-plus-three-evaluation-plus-selection-plus-resource topology, so no scientific third-seed artifact can be combined with seeds 17 and 42.

`runs/seed2026-smoke/` is a checksum-valid end-to-end smoke run. Its command explicitly used `--smoke-max-steps 1` and `--smoke-max-items-per-benchmark 2`; it is workflow evidence only and must not enter effect estimates, bootstrap intervals, or cross-seed summaries. No post-seed-42 continuation decision formally authorizing or stopping seed 2026 is currently recorded. The remaining budget after seed 42 was 37.8172 GPU-hours, but that is not a continuation/stop decision.

If seed 2026 completes with a null selection, that is still a complete valid outcome. It adds one more capability-rejected visible/capability result; it does not authorize held-out reveal or post-hoc checkpoint selection.
...
3. **Two-seed gap:** the old issue anticipated one or three completed seeds. A formal stop after seed 42 would leave two completed seeds and needs an explicitly valid reporting path.
...
- If seed 2026 is formally stopped, is a two-seed report acceptable? If it completes, how should a third null selection be integrated without changing the protocol?
...
- `docs/issue-16-recovery-boundaries-decision.md` through `docs/issue-22-seed-42-execution-decision.md`
- Closed GitHub issue #14 and open GitHub issue #23
